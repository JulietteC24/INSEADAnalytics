...
1. We found 11 correlations higher than 0.5:
3-9, 9-26, 13-14, 14-15, 16-17, 16-18, 16-20, 16-28, 20-27, 20-28,  27-28. 

Yes, the high correlations make sense. From our analysis, the questions can be grouped. For instance, questions 3, 9, and 26 all refer to boats representing a status symbol for the customer. For questions 13-17, these questions all refer to the customers’ knowledge on boats, on right technology and accessories, and on worth. Questions 16, 18, 20, 27, and 28 pertain to the customer’s view of boating as his or her passion (“how passionate the customer is towards boating”). 

2. The high correlations then imply that these questions are related to each other, and because they are related, they have low individual contribution to or impact on the model.
...
1. This table shows the new components created and ranks them based on how much of the variability of the data they explain. It also shows the eigenvalues of the component and percentage of explained variance (eigenvalue/nb of components). The scree plot is a visual representation of each component's eigenvalue from highest to lowest. 
They allow us to identify the components that are most relevant to explain the data and thus select the number of components we want to include in the dimensionality reduction. 

2. It sorts the components from highest eigenvalue to lowest so it will always be decreasing. 

3. We want to see few components that explain a high percentage of the data i.e. that have high eigenvalues. This allows us to reduce the data dimensionally while preserving as much information as possible. 
...
1. For the first factor (component 1), the questions with significant values refer to boats as a status symbol for the customer. For the second factor (component 2), questions pertain to lifestyle where the customer treats their boats as rewards for their hard work (i.e., needing to go on an adventure). These two factors were considered the first and second component as they heavily influence the behavior of the customers towards their boats, how they value boats, which makes a lot of business sense because these affect willingness to pay and/or customer elasticity.

2. We should choose 5 factors. 
a) Each of these factors have an eigenvalue that's higher than 1, which means they each explain a higher amount of variability than the average factor. The cumulative variability they explain is higher than 50%.
b) These factors are easily interpretable (see question 3). 
c) The first 5 factors capture the key drivers of individuals when it comes to boat purchases and boating preferences. Having more than 5 would risk oversegmentation.

3. Looking at the questions within each factor, the significant ones were considered, and the following interpretations were derived. For the first factor, having high eigenvalue and explanatory power, refers to the value of boats for each customer if they treat boats as a status symbol or not. For the second factor, the questions pertain to lifestyle or how the boats will be used or is used by the customers. For the third factor, questions pertain to how customers appreciate boat quality and brand. For the fourth factor, the questions determine the level of expertise of the customers, apart from the appreciation of customers on boat quality. Lastly, the fifth factor has questions on practicality. These factors or grouping of questions help in focusing on the key behavior of customers.

4. Data science techniques are very helpful in deriving useful insights from the data that would be otherwise be difficult to digest and explain to the business. In this specific case, these techniques enables us to intrepret the customer responses to a survey and understand what factors customer appreciate when making a purchase decision.
...
This customer has high values for factor 3, 1, and 2 (ordered). This means the customer appreciates boats, aiming for high-quality and/or good brand. The boat is treated as a status symbol for the customer and part of their lifestyle, but the customer is not necessarily an expert in technical aspect of owning a boat, and therefore would be more inclined to buy a boat because of brand instead of functionality (not an economical buyer)

*Observation 10*

The customer has high value for factor 3 where the customer has high appreciation for boats especially for high quality ones. Boat is part of the customer’s lifestyle, the thing that they reward themselves with. However, the customer is not necessarily an expert when it comes to boats, therefore the customer is more economical as compared to customer 1 (observation 1).

2. 

Factor 1: Q.9

Factor 2: Q.18 or 21 (we chose 21)
...
# Please use numbers, not column names, e.g. c(1:5, 7, 8) uses columns 1,2,3,4,5,7,8
segmentation_attributes_used = c(10,22,5,12,3) #c(10,19,5,12,3) 
...
#rect.hclust(dendroNetwork, k=numb_clusters_used, border="red") 
```
...
#change this variable to change different clustering methods (from hclust to kmeans)
profile_with = "hclust"
...
1. The numbers in the last table indicate the % deviation from the mean of the population that each segment exhibits for a given question. Ie. for Question 1.1 Segment 1's average value is equal to the mean of the population while Segment 5 is on average 7% less than the mean. 

2. The key features are the initial attitude questions (columns 2 to 30) and the final brand equity questions (columns 52-81). You can see that these features have clearly separate levels for each segments while there is much more overlap in the other features (columns 31-51).

Segment 1: Q1.11 (-0.38) and Q7.1 (-0.17)

Segment 2: Q1.11 (0.25), Q17 (0.25) and Q1.2 (-0.23)

Segment 3: Q1.9 (0.45), Q1.2 (0.35), Q18 (-0.27) and Q1.26 (0.27) 

Segment 4: Q17 (-0.19), Q1.4 (-0.16) and Q1.9 (-0.15) 

Segment 5: Q1.9 (-0.42), Q1.11 (0.22), Q15 (0.18) and Q1.2 (-0.18)
...
1. There are 5 segments that we selected in our market.  
a) The statistical reason that we chose this number of segments is that 5 segments is where the "elbow" of the distance plot lies. Meaning, that the distance between the segments becomes much smaller as you increase the number of segments beyond this point, and 5 segments is the optimal point where increasing the number of segments would not yield a significantly different result. This number of segments was also robust in the sense that the segments created were similar when we ran both hclust and kmeans methods.   
b) The interpretation of these segments is simple and clear. There are decisive profiles for each segment. They have differing price sensitivities, uses for their boats and attitudes towards boating in general.   
c) The 5 segments we discovered are useful from a business sense in that there are clear types of boats that suit each segment. In addition, 5 different kinds of boats are a reasonable number for the company to consider.

2. 
Segment 1 is not boat savy and describes people who do not go out on the water alone. 
Segment 2 is boat savy and are brand loyal - they are brand conscious and not very price sensitive.
Segment 3 is very status-oriented, they want a powerful boat regardless of the brand but they are price conscious at the same time.
Segment 4 is well informed, they do not buy boats for status or based on the brand but rather on other characteristics - they are not brand loyal. 
Segment 5 represents true boating aficionados, they do not care about status at all and do their own maintenance, they are not very price-conscious and are slightly older than the general population surveyed.


3. When we changed the number of factors, increasing the number of components from 5 to 6 and 7, the results in terms of segmentation did not yield significant difference. This is as expected as we saw this from one of the analysis where adding component 6 and 7 only add 5.92% (cumulatively) explanation.

4. Our recommendation to CreeqBoat Company would be to target Segment 4. This is a relatively large segment so the market size is attractive. Their profile is that they are not brand loyal and they are not status conscious. This means that they can be easily convinced to switch boats and do not need a high brand value in order to pick a boat. Thererfore, marketing expense does not need to be especially high to make inroads with this segment.
Another opportunity would be to invest in brand marketing to seduce more customers in Segment 2; this segment is brand loyal and not very price sensitive, which means that if CreeqBoat can reach high brand awareness and recognition they will be able to sell them pricier boats.
CreeqBoat can also invest in developing a boat with a powerful motor that is at a lower price point to seduce segment 3, who are very status oriented but price conscious. 
Another opportunity would be to develop a boat targeted at segment 5; this could be a well-thought out boat built to last with good performance and specifications that could speak to an knowledgeable audience.
The largest segment, segment 1 is not a very precise segment; it probably represents the average boat buyer that is not very knowledgeable or savy. CreeqBoat can continue to address this large population segment with regular marketing efforts and boat line. 
